function load_kct(path::String)
    open(path, "r") do io
        ...
        kct = load(io, version=Val(version))
        return kct
    end
end
...
## V1.1 ##
function Base.write(io::IO, kct::NeoKCT{K, Ab}; version::Val{1.1}) where {K, Ab<:Alphabet}
...
function Base.write(io::IO, k_elem::K_Element{K, Ab, C}; version::Val{1.0}) where {K, Ab<:AAAlphabet, C}
    write(io, K)
